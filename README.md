...
| В карточках «Quattro … - ???????» | Кириллица в БД испорчена. `git pull`, затем **`php scripts/repair-encoding.php`** (из корня проекта) или `.\start-dev.ps1` после обновления. Обновите страницу Ctrl+F5 |
